...
const EncounterListDataTable: React.FC<EncounterListProps> = ({
  encounters,
  visitUuid,
}) => {
  ...
  return encounters.length > 0 ? (
                  ...
                  <React.Fragment key={row.id}>
                    ...
                    {row.isExpanded && (
                      <TableExpandedRow
                        ...
                        style={{
                          paddingLeft: isDesktop(layout) ? "3rem" : "4rem",
                        }}
                        colSpan={headers.length + 2}
                      >
                        <div style={{ marginLeft: headerWidth }}>
                          <EncounterObservations
                            observations={encounters[i].obs}
                          />
                        </div>
                      </TableExpandedRow>
                    )}
    ...
    <div className={styles.encounterEmptyState}>
      ...
      <p className={classNames(styles.bodyLong01, styles.text02)}>
        {t(
          "thereIsNoInformationToDisplayHere",
