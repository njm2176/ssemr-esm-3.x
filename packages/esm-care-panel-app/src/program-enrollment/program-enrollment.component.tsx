...
const ProgramEnrollment: React.FC<ProgramEnrollmentProps> = ({
  ...
  const headers = useMemo(
    () =>
      Object.entries(
        programDetailsMap[programName] ?? { ...shareObjProperty }
      ).map(([key, value]) => ({
        key,
        header: value,
      })),
    ...
    launchPatientWorkspace("patient-form-entry-workspace", {
      ...
      formInfo: {
        ...
        additionalProps:
          {
            enrollmenrDetails: {
              dateEnrolled: new Date(enrollment.dateEnrolled),
              uuid: enrollment.enrollmentUuid,
            },
          } ?? {},
      },
    });
      ...
      <div className={styles.tileWrapper}>
        <DataTable size="sm" useZebraStyles rows={rows} headers={headers}>
          {({ rows, headers, getHeaderProps, getRowProps, getTableProps }) => (
            <TableContainer
              title={t("EnrollmentDetails", "Enrollment History")}
              description=""
            >
                        ...
                        {isEmpty(orderedEnrollments[index]?.dateCompleted) && (
                          <OverflowMenu size="sm" flipped>
                            <OverflowMenuItem
                              ...
                              onClick={() =>
                                handleEditEnrollment(orderedEnrollments[index])
                              }
                            />
                            <OverflowMenuItem
                              isDelete
                              ...
                              onClick={() =>
                                handleDiscontinue(orderedEnrollments[index])
                              }
                            />
                          </OverflowMenu>
                        )}
