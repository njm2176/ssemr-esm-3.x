...
import ProgramSummary, {
  ProgramSummaryProps,
} from "./program-summary.component";
...
describe("ProgramSummary Component", () => {
  ...
  it("displays HIV program details correctly", async () => {
    ...
    expect(
      screen.getByText(`(${mockFormatDate(mockProgram.HIV.cd4PercentDate)})`)
    ).toBeInTheDocument();
    expect(screen.getByText("Last WHO stage")).toBeInTheDocument();
    expect(screen.getByText("Regimen")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.HIV.lastEncDetails.regimenShortDisplay)
    ).toBeInTheDocument();
  ...
  xit("displays TB program details correctly", async () => {
    ...
    expect(
      screen.getByText(mockProgram.TB.tbTreatmentNumber)
    ).toBeInTheDocument();
    expect(screen.getByText("Disease classification")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.TB.tbDiseaseClassification)
    ).toBeInTheDocument();
    expect(screen.getByText("Patient classification")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.TB.tbPatientClassification)
    ).toBeInTheDocument();
  });
  ...
  it("displays MCHMOTHER program details correctly", async () => {
    ...
    expect(
      screen.getByText(mockProgram.mchMother.hivStatus)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `(${mockFormatDate(mockProgram.mchMother.hivStatusDate)})`
      )
    ).toBeInTheDocument();
  ...
  it("displays MCHCHILD program details correctly", async () => {
    ...
    expect(screen.getByText("HIV Status")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.mchChild.hivStatus)
    ).toBeInTheDocument();
    expect(screen.getByText("HEI Outcome")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.mchChild.heiOutcome)
    ).toBeInTheDocument();
    expect(screen.getByText("Milestones Attained")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.mchChild.milestonesAttained)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        mockFormatDate(mockProgram.mchChild.milestonesAttainedDate)
      )
    ).toBeInTheDocument();
    expect(screen.getByText("Current feeding option")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.mchChild.currentFeedingOption)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        mockFormatDate(mockProgram.mchChild.currentFeedingOptionDate)
      )
    ).toBeInTheDocument();
    expect(screen.getByText("Current prophylaxis used")).toBeInTheDocument();
    expect(
      screen.getByText(mockProgram.mchChild.currentProphylaxisUsed)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        mockFormatDate(mockProgram.mchChild.currentProphylaxisUsedDate)
      )
    ).toBeInTheDocument();
  });
});
