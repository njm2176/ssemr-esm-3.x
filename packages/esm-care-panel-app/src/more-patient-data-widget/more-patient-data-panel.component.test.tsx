...
describe("CarePanel Component", () => {
  xit("renders without crashing", () => {
    jest
      .spyOn(require("../hooks/useEnrollmentHistory"), "useEnrollmentHistory")
      .mockReturnValue({
  ...
  xit("displays loading skeleton when isLoading is true", () => {
    ...
    expect(
      screen.getByTestId("mocked-structured-list-skeleton")
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("mocked-program-summary")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("mocked-program-enrollment")
    ).not.toBeInTheDocument();
  });
  ...
  xit("displays error message when isError is true", () => {
    ...
    expect(
      screen.getByText("Error loading program enrollments")
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("mocked-structured-list-skeleton")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("mocked-program-summary")
    ).not.toBeInTheDocument();
